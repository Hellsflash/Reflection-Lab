...
public class Spy
{
    ...
    public string CollectGettersAndSetters(string className)
    {
        ...
        MethodInfo[] classMetods =
            classType.GetMethods(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);

        StringBuilder sb = new StringBuilder();

        foreach (MethodInfo method in classMetods.Where(m => m.Name.StartsWith("get")))
        {
            sb.AppendLine($"{method.Name} will return {method.ReturnType}");
        }
        foreach (MethodInfo method in classMetods.Where(m => m.Name.StartsWith("set")))
        {
